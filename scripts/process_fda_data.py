...
從 EMA 下載 Article 57 及 Medicines Report Excel，作為德國已授權藥品的
資料來源。BfArM AMIce 僅提供網頁查詢，無批量下載。

使用方法:
    uv run python scripts/process_fda_data.py

資料來源:
    主要: EMA Medicines Report (集中授權藥品，含德國)
    ...
    參考: BfArM AMIce (僅網頁查詢)
          https://portal.bfarm.de/amguifree/

產生檔案:
    data/raw/de_fda_drugs.json
"""
...
def download_ema_data(output_path: Path) -> Path:
    ...
    嘗試兩個 EMA 資料來源：
      1. Medicines Report: 集中授權程序的藥品清單 (~881 KB)
    ...
    raise FileNotFoundError(
        ...
        f"  1. 前往上述頁面\n"
        f"  2. 或直接前往 https://www.ema.europa.eu/en/human-regulatory-overview/\n"
        f"     post-authorisation/data-medicines-iso-idmp-standards-post-authorisation/\n"
        f"     public-data-article-57-database\n"
        f"  3. 下載 Article 57 Product Data Excel\n"
        f"  4. 將檔案放置於: {output_path.parent}/ema_article57_product_data.xlsx\n\n"
        f"方法 C: BfArM AMIce (手動查詢匯出)\n"
        f"  1. 前往 https://portal.bfarm.de/amguifree/\n"
        f"  2. 搜尋並手動匯出藥品資料\n"
        f"  3. 將匯出檔案放至 {output_path.parent}/\n\n"
...
def find_data_file(raw_dir: Path) -> Path | None:
    ...
    candidates = [
        ...
        raw_dir / "bfarm_arzneimittel.csv",
        raw_dir / "bfarm_arzneimittel.json",
        raw_dir / "bfarm_arzneimittel.xlsx",
        raw_dir / "amice_export.csv",
        raw_dir / "amice_export.json",
        raw_dir / "pharmnet_export.csv",
    ]
...
def process_data_file(input_path: Path, output_path: Path) -> Path:
    ...
    if suffix == ".csv":
        ...
        try:
            ...
            # 自動偵測表頭列：找到包含 'Category' 或 'Name of medicine' 的列
    ...
    # ── 若為 EMA 資料，嘗試篩選德國相關記錄 ──
    if is_ema:
        df = _filter_ema_for_germany(df)
...
def _filter_ema_for_germany(df: pd.DataFrame) -> pd.DataFrame:
    """從 EMA 資料中篩選德國相關記錄

    EMA Article 57 資料包含 'Country' 或 'Authorisation country' 欄位。
    EMA Medicines Report 為集中授權，在所有 EU 成員國有效。
    ...
    if country_cols:
        ...
        # 檢查是否有德國記錄
        de_mask = df[country_col].str.contains(
            r"Germany|Deutschland|DE\b",
            case=False, na=False, regex=True,
        )
        de_count = de_mask.sum()
        print(f"德國相關記錄: {de_count:,} / {len(df):,}")

        if de_count > 0:
            df = df[de_mask].copy()
            print(f"已篩選為德國記錄: {len(df):,} 筆")
        else:
            print("未找到德國特定記錄，保留所有記錄")
...
def print_statistics(df: pd.DataFrame, config: dict):
    ...
    if ingredients_field and ingredients_field in df.columns:
        ...
        ing_like = [
            ...
            or "wirkstoff" in c.lower() or "ingredient" in c.lower()
        ]
        if ing_like:
            col = ing_like[0]
...
def main():
    print("=" * 60)
    print("處理德國 BfArM/EMA 藥品資料")
    print("=" * 60)
    print()
    print("注意: BfArM AMIce 入口無批量下載功能。")
    print("使用 EMA 資料作為替代來源 (集中授權藥品在德國有效)。")
    print()

    base_dir = Path(__file__).parent.parent
    raw_dir = base_dir / "data" / "raw"
    output_path = raw_dir / "de_fda_drugs.json"
