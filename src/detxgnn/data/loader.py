...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/de_fda_drugs.json

    Returns:
        包含所有藥品的 DataFrame

    Raises:
        FileNotFoundError: 找不到 BfArM 資料檔案時，提供下載指引
    """
    if filepath is None:
        # loader.py -> data -> detxgnn -> src -> DETxGNN
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "de_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到 BfArM 藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
            f"1. 執行: uv run python scripts/process_fda_data.py\n"
            f"   (需先手動從 AMIce 入口匯出資料至 data/raw/)"
        )
