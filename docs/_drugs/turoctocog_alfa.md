...
| 項目 | 內容 |
|------|------|
| 原始適應症 | 資料缺口（DrugBank original_indications 為空；台灣Not marketed無許可證資料；公開已知用途為 Factor VIII 替代療法） |
...
補充說明：本 Evidence Pack 標記 BfArM package insert warnings/contraindications為 **Blocking 等級資料缺口（DG001）**，於此缺口補齊前，本藥物Cannot proceed to S1 safety screening階段；作用機轉資料（DG002）亦屬 High 等級缺口，Limits mechanistic-link analysis之可靠度。
...
- 台灣Not marketed、無許可證資料，且 TFDA 安全性資料為 Blocking 等級缺口，尚無法完成 S1 安全性初評。

**若要推進，需補足：**
- TFDA 仿單警語與禁忌資料（DG001，Blocking，需下載官方仿單 PDF 解析）
- DrugBank 作用機轉（MOA）詳細資料（DG002，High，需Query the DrugBank API）
